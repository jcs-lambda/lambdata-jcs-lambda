...
def tvt_split(df, target:str='', random_state=None):
    ...
    train, test = train_test_split(df, test_size=0.2, stratify=stratify, random_state=random_state)

    if stratify is not None:
        stratify = train[target]

    train, val = train_test_split(train, test_size=0.2, stratify=stratify, random_state=random_state)
...
def barplot_feat_by_target_eq_class(feature:str, target:str, target_class, dataframe, ylim=0.7, figsize=(9, 6)):
    ...
    assert target_class in dataframe[target], f'TARGET CLASS: {target_class} NOT FOUND IN DATAFRAME[\'{target}\']'
    ...
    plt.axhline(y=dataframe[target].value_counts(normalize=True)[target_class], color='red')
